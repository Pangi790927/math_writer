...
-- |
-- | declare(owner: string, name: string, fields: {[string] = string}, opts: table) -> shape
...
-- |
-- | shape.check_name(value: string, what: string) -> value
...
-- |
-- | shape.check_keys(t: table, what: string)   -> t
-- | shape.check_covers(t: table, what: string) -> t
...
-- |     the declaration. check_keys is also how an ALREADY-BUILT options
-- |     literal is checked, which a seal cannot police; nil passes it.
-- |
-- | shape.is(t: any)                        -> boolean
-- | shape.check(t: any, what: string)       -> t
...
-- |
-- | type_of(t: any)                         -> "owner.name" | nil
-- |     Which sealed type any value is, for an error message.
-- |
-- | all()                                   -> {shape, ...}
...
--[[ @brief Which sealed type a value is, as "owner.name", or nil for anything that is not one.
-- |
-- | FOR ERROR MESSAGES, chiefly: when a check fails, saying which container arrived is most of the
-- | diagnosis. ASKING IS ALLOWED ABOUT ANYTHING, so a plain table, a nil or a number answer nil
-- | rather than throwing.
-- |
-- | @param t  any
-- | @return string | nil - the declared "owner.name", read off the metatable
-- |
-- | @date 2026-09-13 17:25
--]]
...
--[[ @brief Declares a container's field set, and returns the shape that seals and checks it.
-- |
-- | THE FIELD LIST IS THE DOCUMENTATION. `fields` maps each name to a one-line description of what
-- | it holds; the descriptions are not used at runtime, deliberately, so there is one list rather
-- | than a list and a comment that can disagree with it.
-- |
-- | ONE METATABLE PER CALL, and that metatable IS the type: a declared field never set reads nil,
-- | an undeclared name raises on read and on write, and `check` compares metatables - so a table
-- | with exactly the right fields but built elsewhere is still refused.
-- |
-- | @details Called once per container, at file scope. A file declares its shapes together near
-- |          the top, since a shape used above its declaration is a nil global to Lua.
-- |
-- | @param owner   string - the module that owns the container, as it appears in `require`;
-- |                named in every error
-- | @param name    string - what the table is called where it is used: "u", "state_doc", "ctx"
-- | @param fields  {[string] = string} - field name -> one-line meaning
-- | @param opts    table | nil -
-- |                  `readonly = true` refuses EVERY write, not only undeclared ones - for a
-- |                  container handed to somebody as input, where writing back would be a side
-- |                  channel instead of a return value. A plugin's ctx is the case.
-- |                  `array = true` lets INTEGER keys through untouched while still policing the
-- |                  string ones. An ast node is the case: `type` and `id` are its whole named
-- |                  surface, and its children are positional and unbounded.
-- | @return shape - `owner`, `name`, `type_name` ("owner.name"), `fields` as given, and `wrap`,
-- |         `check_keys`, `check_name`, `check_covers`, `is`, `check`. Also appended to all().
-- |
-- | @date 2026-09-13 17:25
--]]
function sealed.declare(owner, name, fields, opts)
    ...
    local shape = {
        ...
        type_name = type_name,
        fields = fields,
        --[[ @brief Seals a table the creator just built. The one thing a creator calls.
        -- |
        -- | TAKES THE TABLE rather than building one: a creator wants to write its initial fields
        -- | as a normal constructor expression, and those writes must happen BEFORE the seal is
        -- | on - otherwise every one of them pays the metamethod.
        -- |
        -- | @param t  table - freshly built; its metatable is replaced
        -- | @return table - `t` itself, now sealed
        -- |
        -- | @date 2026-09-13 17:25
        --]]
        wrap = function(t)
        ...
        --[[ @brief Every key of an ALREADY-BUILT table, against the declaration.
        -- |
        ...
        -- | present before this ever sees it, misspelt ones included, and `setmetatable` after the
        -- | fact never fires on them. That is a property of Lua, not a choice.
        -- |
        -- | @param t     table | nil - nil checks nothing: an absent options table is every option
        -- |              at its default
        -- | @param what  string | nil - names the table in the error, instead of the shape's name
        -- | @return table | nil - `t`, so it can stand at the top of a function
        -- | @throws when `t` is not a table, or has a key the declaration does not
        -- |
        -- | @note Two files had written this loop out by hand before it was put here. Author,
        -- |       2026-09-12: "what is this, why not the sealed?".
        -- |
        -- | @date 2026-09-13 17:25
        --]]
        ...
        --[[ @brief ONE NAME against the declaration, for a value that must be one of a fixed set.
        -- |
        -- | A declared set of names with a line each is exactly what `declare` already takes, and
        -- | "is this one of them" is the same question `check_keys` asks of a whole table - so it
        -- | is the same mechanism rather than a hand-written `assert(SOME_TABLE[x], ...)`. For an
        -- | enum-shaped value: a parse mark's status, a field name being classified.
        -- |
        -- | @param value  string - the name to check
        -- | @param what   string | nil - names the value in the error
        -- | @return string - `value`
        -- | @throws when `value` is not declared
        -- |
        -- | @note There were four hand-written asserts before this existed, each with its own
        -- |       wording. Author, 2026-09-12, on the first: "you did that aberation starting from
        -- |       here?".
        -- |
        -- | @date 2026-09-13 17:25
        --]]
        ...
        --[[ @brief The REVERSE of check_keys: every declared name has an entry in `t`.
        -- |
        -- | ONE QUESTION FROM BOTH ENDS - "do these sets agree" - and both ends are needed where a
        -- | second table mirrors the declaration. A status with no colour is not a quieter mark, it
        -- | is an invisible one; a colour with no status behind it means the two have drifted.
        -- | Neither direction is implied by the other.
        -- |
        -- | @param t     table - the mirroring table; extra keys in it are not this check's
        -- |              business
        -- | @param what  string | nil - names the mirroring table in the message, since it is that
        -- |              table which is missing an entry, not this declaration
        -- | @return table - `t`
        -- | @throws naming the first declared name `t` has no entry for
        -- |
        -- | @date 2026-09-13 17:25
        --]]
        ...
        --[[ @brief Is this that container? One metatable comparison, cheap on every call.
        -- | @date 2026-09-13 17:25
        --]]
        is = function(t)
        ...
        --[[ @brief Asserts that `t` is this container, for the top of a function that takes one.
        -- |
        -- | WHAT THIS CATCHES that the field seal cannot: handing a function the WRONG container.
        -- | Box states share field names - `state_text` and `state_definition` both have `undo` -
        -- | so passing one where the other belongs reads every field it shares and goes quietly
        -- | wrong.
        -- | The seal only sees names, and those names are all declared.
        -- |
        -- | @param t     any
        -- | @param what  string | nil - the parameter's name, added to the message
        -- | @return table - `t`
        -- | @throws naming both types when `t` is some other sealed container, "an unsealed table"
        -- |         for a plain one, else its Lua type. Level 3, so the position reported is the
...
--[[ @brief Every shape declared so far, in declaration order.
-- |
-- | POPULATED BY REQUIRE ORDER, so a container appears here only once its file has been loaded -
-- | which is why a caller that wants all of them requires the modules first.
-- |
-- | @return {shape, ...} - the LIVE list, not a copy: read it, do not write it
-- |
-- | @date 2026-09-13 17:25
--]]
function sealed.all()
